...
rectification and DAC applications*/
{
	...
	//define frequency
	//example: we require 2ms pulse every 50 hz (20 ms)
	//count in ICR1= 20ms/0.5us = 40,000
	//then to ICR1= 39,999
	ICR1 = 39999;
	
	//define fixed pulse width
	//count to OCR1= 2ms/0.5us= 4000
	OCR1A = ICR1 - 4000;
	
	//TCCR1A &= 0 << CS10; //stop counter
	//example for varying the pulse width
	/*
	while (1)
	{
		OCR1A = ICR1 - 4200;
...
void timer2_CTCmode(void)
{
	...
	*/
	...
	/* uncommnented for polling method
	while (1)
	{
...
{
	//use PB3 (OC2A) as output (define PB3--> output in ports.c)
	//define PHASE CORRECT PWM (pag.164)
	TCCR2A |= 1 << WGM20;
	...
	OCR2A = 51;
	/* uncommnented for varying the duty cycle
